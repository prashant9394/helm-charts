...
| [TEE Attestation](usecases/tee-attestation/deployment.md) | TEE Attestation use case involves SGX and TDX attestation. TDX attestation provides a remote Attestation Service that maintains a database of expected measurements (or “policies”), and compares the actual boot-time measurements of a Trusted Domain (TD) against its database to provide an assertion that the VM booted into a “trusted” or “untrusted” state. SGX ECDSA attestation is the process that allows an application (relying party) to verify that a remote piece of code and data that it's interacting with is contained in a genuine Intel SGX enclave. | *Certificate Management Service (CMS)*<br/><br/>*Authentication and Authorization Service (AAS)*<br/><br/>*Tee Caching Service(TCS)*<br/><br/>*Feature Discovery Agent(FDA)*<br/><br/>*Key Broker Service(KBS)*<br/><br/>*Attestation Policy Service(APS)*<br/><br/>*Quote Verification Service(QVS)* |
| [TEE Orchestration](usecases/tee-orchestration/deployment.md) | The TEE Attestation Infrastructure can optionally push the TEE information on compute nodes to cloud orchestrators so that TEE workloads (like SKC) can be scheduled on compute nodes that support TEE. | *Certificate Management Service (CMS)*<br/><br/>*Authentication and Authorization Service (AAS)*<br/><br/>*Tee Caching Service(TCS)*<br/><br/>*Feature Discovery Agent(FDA)*<br/><br/>*Feature Discovery Service(FDS)*<br/><br/>*Integration Hub(IHUB)*<br/><br/>*ISecL Scheduler*<br/><br/>*ISecL Controller* |
| [TEE Attestation Orchestration](usecases/tee-attestation-orchestration/deployment.md) | TEE Attestation Orchestration use case involves both TEE Attestation and Orchestration usecases. The TEE Attestation Infrastructure pushes the TEE information on compute nodes to cloud orchestrators so that TEE workloads (like SKC) can be scheduled on compute nodes that support TEE. | *Certificate Management Service (CMS)*<br/><br/>*Authentication and Authorization Service (AAS)*<br/><br/>*Tee Caching Service(TCS)*<br/><br/>*Feature Discovery Agent(FDA)*<br/><br/>*Key Broker Service(KBS)*<br/><br/>*Attestation Policy Service(APS)*<br/><br/>*Quote Verification Service(QVS)*<br/><br/>*Feature Discovery Service(FDS)*<br/><br/>*Integration Hub(IHUB)*<br/><br/>*ISecL Scheduler*<br/><br/>*ISecL Controller* |
